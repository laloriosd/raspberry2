...
# ----------------------------
# Utils de limpieza (tu código original)
# ----------------------------
...
# reemplazo de calcular_edad para usar fecha de referencia del usuario
...
def crear_ids(df):
    # Mantengo todas las ids que usaste en tu código original
...
# ----------------------------
# Helper: header con logo y gradiente (ahora incluye fecha)
# ----------------------------
def dibujar_header(logo_path=None, titulo="Altas y Cambios de Beneficiarios", subtitulo="Somos tu aliado en protección", fecha_str=None):
...
# ----------------------------
# UI: pedir fecha primero (obligatoria)
# ----------------------------
st.write("")  # espacio
st.title("Altas y Cambios de Beneficiarios")
fecha_proceso = st.date_input("📅 Selecciona la fecha del proceso", value=date.today())
fecha_txt = fecha_proceso.strftime("%Y-%m-%d")

# Header (intenta usar logo subido o ruta por defecto)
...
if logo_subido:
    ...
    dibujar_header(ruta_logo_temporal, fecha_str=fecha_txt)
else:
    if os.path.exists(RUTA_LOGO_POR_DEFECTO):
        dibujar_header(RUTA_LOGO_POR_DEFECTO, fecha_str=fecha_txt)
    else:
        dibujar_header(None, fecha_str=fecha_txt)

st.markdown("## Subir archivos para procesar")
archivo_semana = st.file_uploader("📌 Archivo de la semana (ALTAS)", type=["xlsx"], key="altas")
archivo_bitacora = st.file_uploader("📘 Archivo histórico (bitácora)", type=["xlsx"], key="hist")

# ----------------------------
# Procesamiento: tu LÓGICA COMPLETA (con uso de fecha_proceso)
# ----------------------------
if archivo_semana and archivo_bitacora:
    ...
    # ==== limpiar histórico (igual que tu script) ====
    mapeo_columnas = {
        'NOMBRE': 'NOMBRE',
    ...
    fecha_limite = fecha_proceso - relativedelta(months=6)  # ahora con fecha de proceso
    validas = altas["FECHA_DE_MOVIMIENTO"].notna().sum()
    if validas == 0:
        st.error("FECHA_DE_MOVIMIENTO no tiene fechas válidas. Revisa el archivo de ALTAS.")
    else:
        ...
        # calcular edad con la fecha que el usuario introdujo
        altas_6["EDAD"] = altas_6["FECHA_DE_NACIMIENTO_BENEFICIARIO"].apply(lambda x: calcular_edad_con_fecha(x, fecha_proceso))
        ...
        # clasificación robusta (mantengo EXACTAMENTE tu lógica)
        ...
        # detectar diferencias (idéntico a tu script)
        hist_key = hist[[
        ...
        # ----------------------------
        # PREPARAR "BAJAS" (hoja requerida)
        # ----------------------------
        # No hay regla explícita de "baja" en tu código original, así que creo una hoja "BAJAS" vacía
        # con las mismas columnas que ALTAS para que esté disponible.
        # Si tienes una columna que identifique bajas, reemplaza la siguiente línea por la condición adecuada.
        if not altas_finales.empty:
            bajas_df = pd.DataFrame(columns=altas_finales.columns)
        else:
            bajas_df = pd.DataFrame()

        # ----------------------------
        # EXPORTAR: workbook con sheets ALTAS y BAJAS + botones individuales
        # ----------------------------
        def to_excel_workbook_bytes(df_altas, df_bajas, fecha_str):
            ...
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                ...
                # opcional: agregar un sheet resumen con conteos
                resumen = pd.DataFrame({
                    "TIPO": ["ALTAS", "CAMBIOS", "REVISION", "YA_EXISTE"],
                    "CUENTA": [len(altas_finales), len(cambios_final), len(revision_out), len(ya_existente_out)]
                })
        ...
        # Graficos: mismo tamaño y lado a lado
        col1, col2 = st.columns(2)

        # Pie chart (izquierda)
        fig1, ax1 = plt.subplots(figsize=(5,4))
        # Mantener un color consistente: usar same color tone for slices (if single color, Matplotlib will vary shade)
        wedges, texts, autotexts = ax1.pie(counts.values(), labels=counts.keys(), autopct="%1.1f%%", colors=[raspberry, "#F2A1AB", "#F6D6DA"])
        ax1.set_title("Distribución general")
        col1.pyplot(fig1)

        # Bar chart (derecha)
        fig2, ax2 = plt.subplots(figsize=(5,4))
        ax2.bar(["Cambios", "Revisión"], [counts["CAMBIOS"], counts["REVISION"]], color=[raspberry, "#F2A1AB"])
        ax2.set_title("Cambios vs Revisión")
        col2.pyplot(fig2)

        # Descargas
        st.subheader("📥 Descargar resultados")
        # Workbook (ALTAS + BAJAS + RESUMEN)
        workbook_bytes = to_excel_workbook_bytes(altas_finales, bajas_df, fecha_txt)
        st.download_button("💾 Descargar workbook (sheets: ALTAS, BAJAS, RESUMEN)",
                           workbook_bytes,
                           file_name=f"RESULTADOS_{fecha_txt}.xlsx",
                           mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

        # Botones individuales (opcionales)
        st.download_button("📘 ALTAS (solo sheet ALTAS)", df_to_excel_bytes(altas_finales), file_name=f"ALTAS_{fecha_txt}.xlsx")
        st.download_button("📙 CAMBIOS (solo)", df_to_excel_bytes(cambios_final), file_name=f"CAMBIOS_{fecha_txt}.xlsx")
        st.download_button("📗 REVISIÓN (solo)", df_to_excel_bytes(revision_out), file_name=f"REVISION_{fecha_txt}.xlsx")
        st.download_button("📕 YA EXISTE (solo)", df_to_excel_bytes(ya_existente_out), file_name=f"YA_EXISTE_{fecha_txt}.xlsx")
